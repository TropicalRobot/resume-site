...
export const ThreeDMarquee = ({ images, className }: ThreeDMarqueeProps) => {
    const imagesArray = Array(6)
        .fill(null)
        .flatMap(() => [...images])
    // Split the images array into 2 equal parts
    const chunkSize = Math.ceil(imagesArray.length / 4)
    const chunks = Array.from({ length: 4 }, (_, colIndex) => {
        const start = colIndex * chunkSize
        return imagesArray.slice(start, start + chunkSize)
    })
                ...
                <div className='size-[1720px] shrink-0 scale-50 sm:scale-75 lg:scale-100'>
                    <div
                        ...
                        {chunks.map((subarray, colIndex) => (
                            <motion.div
                                animate={{ y: colIndex % 3 === 0 ? 100 : -100 }}
                                transition={{
